...
//
// This file implements a simple IR generation targeting MLIR from a Module 
// AST for the Toy language.
//
//===----------------------------------------------------------------------===//
...
///
/// This will emit operation that are specific to the Toy language, preserving
...
private:
    ...
    llvm::ScopedHashTable<llvm::StringRef, std::pair<mlir::Value, VarDeclExprAST *>>
        symbolTable;
    using SymbolTableScopeT =
        llvm::ScopedHashTableScope<llvm::StringRef,
    ...
    /// Helper conversion for a AST location to an MLIR location.
    mlir::Location loc(const Location &loc) {
        return mlir::FileLineColLoc::get(builder.getStringAttr(*loc.file), loc.line,
                                         loc.col);
    }

    /// Declare a variable in the scope, return success if the variable
    /// wasn't declared yet.
    ...
    /// Return the struct type that is the result of the fiven expression, or null
    ...
    mlir::DenseElementsAttr getConstantAttr(NumberExprAST &lit) {
        ...
        // This is the actual attribute taht holds the list of values for this
        // tensor literal.
        return mlir::DenseElementsAttr::get(dataType,
                                            llvm::ArrayRef(lit.getValue()));
    }
    /// Emit a onstant for a struct literal. It will be emitted as an array of
    ...
    /// Recursive helper function to accumlate the data that compose an array
    /// literal. It flattens the nested structrue in the supplied vector. For
    /// example with this array:
    ///   [[1, 2], [3, 4]]
    /// we will generate:
    ///   [1, 2, 3, 4]
    /// Individual numbers are represented as doubles.
    /// Attributes are the MLIR attaches constant to operations.
    ...
    mlir::Value mlirGen(CallExprAST &call) {
        ...
        // Otherwise this is a call to user-defined function. Calls to
        // user-defined function are mapped to a custom call that takes the callee
        ...
        FuncOp calledFunc = calledFuncIt->second;
    ...
    mlir::Value mlirGen(ExprAST &expr) {
        ...
        default:
            emitError(loc(expr.loc()))
                << "MLIR codegen encounterd an unhandled expr kind '"
    ...
    /// Future expression will be able to reference this variable through symbol
    /// table lookup.
    mlir::Value mlirGen(VarDeclExprAST &vardecl) {
        ...
        // declared with specific shape, we emit a "reshape" operation. It will get
    ...
    /// Build a tensor from a list of shape dimensions.
