...
static mlir::LogicalResult verifyConstantForType(mlir::Type type,
    ...
    if (type.isa<mlir::TensorType>()) {
        ...
        if (!attrValue)
            return op->emitError("constant of TensorType must be initialized by "
                                 "a DenseFPElementsAttr, got ") << opaqueValue;
    ...
    auto attrValue = opaqueValue.dyn_cast<mlir::ArrayAttr>();
...
struct StructTypeStorage : public mlir::TypeStorage {
    ...
    /// istance. This type will be used when uniquing an istance of the type
    ...
    /// have hash functions available, so we could just omit this entrirely.
    static llvm::hash_code hashKey(const KeyTy &key) {
        return llvm::hash_value(key);
    }

    /// Define a contruction function for the key type from a set of parameters.
    ...
    /// This method takes an instance of a storage allocator, and an insatnce of a
...
mlir::Type ToyDialect::parseType(mlir::DialectAsmParser &parser) const {
    ...
    // speciailization of LogicalResult that auto-converts to a `true` boolean
    // value on failure  to allow for chaining, but may be used with eplicit
