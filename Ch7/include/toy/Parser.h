...
//
// This file implements the Parser of the Toy language. It process the
...
private:
    ...
    std::unique_ptr<ExprAST> parseTensorLiteralExpr() {
        ...
        if (llvm::any_of(values, [](std::unique_ptr<ExprAST> &expr)
            ...
            // Append the nested dimensions to the current literal.
    ...
    std::unique_ptr<VarDeclExprAST> parseTypedDeclaration(llvm::StringRef typeName,
                                                          bool requiresInitializer,
                                                          const Location &loc) {
    ...
    /// Parse a variable declaration, its starts with a `var` keyword followed by
    /// and identifier and an optional type (shape specification) before the
    /// initializer.
    /// decl ::= var indentifier [ type ] = expr
    std::unique_ptr<VarDeclExprAST> parseDeclaration(bool requiresInitializer) {
        ...
        // Parse the type name.
        if (lexer.getCurToken() != tok_identifier)
            return parseError<VarDeclExprAST>("type name", "in variabledeclaration");
        auto loc = lexer.getLastLocation();
    ...
    std::unique_ptr<ExprASTList> parseBlock() {
        ...
        // Ingore empty expressions: swallow squences of semicolons.
        ...
        while (lexer.getCurToken() != '}' && lexer.getCurToken() != tok_eof) {
            ...
            // Ignore empty expression: swallow sequences of semicolons.
            while (lexer.getCurToken() == ';')
                lexer.consume(Token(';'));
        }
