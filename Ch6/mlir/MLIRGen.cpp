...
private:
  ...
  mlir::toy::FuncOp mlirGen(FunctionAST &funcAST) {
    ...
    if (failed(mlirGen(*funcAST.getBody()))) {
      function.erase();
      return nullptr;
    }
    ...
    // If this function isn't main, then set the visiblity to private.
  ...
  mlir::Value mlirGen(BinaryExprAST &binop) {
    ...
    // Derive the operation name from the binary operator. At themoment we only
    // support '+' and '*'.
    switch (binop.getOp()) {
    case '+':
  ...
  /// See documentation on [Attribute](LangRef.md#attributes) for more details.
  /// Heres is an excerpt:
  ///
  ...
  mlir::Value mlirGen(VarDeclExprAST &vardecl) {
    ExprAST *init = vardecl.getInitVal();
    if (!init) {
    ...
    // with specific shape, we emit a "reshape" operation. It will get optimized
    // out later as needed.
  ...
  mlir::LogicalResult mlirGen(ExprASTList &blockAST) {
    ...
    for (auto &expr : blockAST) {
      ...
      // print. These can only appear in block list and bot in nested
      // expressions.
  ...
  /// Build a MLIR type from a Toy AST variable type (forward to the generic
  /// getType above).
...
mlir::OwningOpRef<mlir::ModuleOp> toy::mlirGen(mlir::MLIRContext &context,
                                               ModuleAST &moduleAST) {
