...
static Value insertAllocAndDealloc(MemRefType type, Location loc,
  ...
  // Make sure to deallocate this alloc at the end of the block. This is fine as
  // toy functions have no control flow.
...
/// corresponding to the operands of the input operation, and range of loop
...
struct ConstantOpLowering : public OpRewritePattern<toy::ConstantOp> {
  ...
  LogicalResult matchAndRewrite(toy::ConstantOp op,
    ...
    // generating a store when recursion hits the base case.
...
struct ReturnOpLowering : public OpRewritePattern<toy::ReturnOp> {
  ...
  LogicalResult matchAndRewrite(toy::ReturnOp op,
                                PatternRewriter &rewriter) const final {
    // During this lowering, we expect that all function calls have been inlned.
    if (op.hasOperand())
      return failure();
...
/// This a partial lowering to affine loops of the toy operations that are
