...
struct PrintOpLowering : public ConversionPattern {
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    Value formatSpecificCst = getOrCreateGlobalString(
    ...
    rewriter.create<LLVM::CallOp>(
        ...
        ArrayRef<Value>({formatSpecificCst, elementLoad}));
...
private:
  ...
  static LLVM::LLVMFunctionType getPrintfType(MLIRContext *context) {
    ...
    auto llvmFnType =
        LLVM::LLVMFunctionType::get(llvmI32Ty, llvmPtrTy, /*isVarArg=*/true);
    return llvmFnType;
  }
...
void ToyToLLVMLoweringPass::runOnOperation() {
  ...
  // patterns used for lowering. At this point of the compliation process, we
  // have a combination of `toy`, `affine`, and `std` operations. Luckily, there
  // are aleady exists a set of patterns to transform `affine` and `std`
  // dialects. These patterns lowering in multiple stages, relying on transitive
  // lowerings. Transtive lowering, or A->B->C lowering, is when multiple
  // patterns must be applied to fully transform an illegal operation into a set
  // of legal ones.
