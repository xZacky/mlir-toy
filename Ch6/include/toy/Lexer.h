...
/// The Lexer is an sbstract base class providing all the facilities that the
...
public:
  ...
  /// Move to the next token in the stream, assreting on the current token
...
private:
  ...
  /// The last value returned by `getNextChar()`. We need to keep it around as
  /// we always need to read ahead one character to decide when to end a token
  /// and we can't put it back in the stream after reading from it.
