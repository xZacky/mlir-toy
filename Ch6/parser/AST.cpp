...
void printLitHelper(ExprAST *litOrNum) {
  ...
  if (NumberExprAST *num = llvm::dyn_cast<NumberExprAST>(litOrNum)) {
    llvm::errs() << num->getValue();
    return;
  }
  LiteralExprAST *literal = llvm::cast<LiteralExprAST>(litOrNum);
...
/// Print a literal, see recursively helper above for implementation.
...
/// Print a module, actually loop over the functions and print them in squence.
...
/// Public API.
void toy::dump(ModuleAST &module) { ASTDumper().dump(&module); }
